...
int main(int argc, char** argv)
{
    ...
    while (ros::ok()) {
        ...
        std::cout<<0.05 * sin(2 * M_PI / 1.5 * time_now_d)<<std::endl;
//        double time_now_d = time_now;
        target_frame.p.data[2] = original_P.p.data[2] + 0.05 * sin(2 * M_PI / 1.5 * time_now_d);
        ur5e->servoj_moveto(target_frame, 1 / control_rate, true);
    ...
    for (unsigned int i=0;i<sizeof(target_frame.p.data)/sizeof(target_frame.p.data[0]);++i)
        ROS_INFO_STREAM(target_frame.p.data[i]);
//    std::cout<<sizeof(target_frame.p.data)/sizeof(target_frame.p.data[0])<<std::endl;
    // record data
    ROS_INFO_STREAM("Recording data");
    std::ofstream data_file;
    std::string filename =  "sin_test_1.txt";
